...
// Rules
const emailRules = [
  (v) => !!v || "Un courriel doit être entré."
]

const passwordRules = [
  (v) => !!v || "Un mot de passe doit être entré."
]
...
const postLogin = async () => {
  try {
    const isFormValid = await loginForm.value.validate().then((formValidity) => {
...
<template>
  <v-form ref="loginForm" @submit.prevent="postLogin">
    <v-card class="mx-auto my-12 pa-12 pb-8" elevation="8" max-width="448" rounded="lg">
      ...
      <v-text-field v-model="email" :rules="emailRules" @update:model-value="verify" density="compact"
        placeholder="Courriel" prepend-inner-icon="mdi-email-outline" variant="outlined"></v-text-field>

      <div class="text-subtitle-1 text-medium-emphasis d-flex align-center justify-space-between">
        Mot de passe
      </div>

      <v-text-field v-model="password" :rules="passwordRules" @update:model-value="verify"
        :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'" :type="visible ? 'text' : 'password'" density="compact"
        placeholder="Entrez votre mot de passe" prepend-inner-icon="mdi-lock-outline" variant="outlined"
