...
export const testUserService = async () => {
  ...
  if (config.debugMode) {
    ...
    try {
      ...
      console.error(`Erreur lors de la récupération de l'utilisateur : ${error.response.statusText}`);
    }
  }
...
export const createUser = async (name, email, password) => {
    try {
        const response = await axios.post(`${config.apiUrl}/signup`, { name, email, password });
        return response.data;
    } catch (error) {
        console.error(`Erreur lors de la création de l'utilisateur : ${error.response.statusText}`);
        throw error;
    }
}
